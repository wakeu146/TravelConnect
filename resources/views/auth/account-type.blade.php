    ...
    <main class="grid min-h-screen lg:grid-cols-2">
        ...
        <section class="flex min-h-screen items-center justify-center px-6 py-12 sm:px-10">
            <div class="w-full max-w-md">
                ...
                <p class="mt-8 text-center text-sm text-[#607985]">{{ __('messages.already_have_an_account') }} <a href="{{ route('login', ['lang' => app()->getLocale()]) }}" class="font-semibold text-[#e76f51] hover:text-[#b94d35]">{{ __('navigation.login') }}</a></p>
            </div>
        </section>
    </main>
</x-auth-layout>
