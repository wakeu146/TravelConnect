<x-auth-layout title="Verify your email | TravelConnect">
    <main class="flex min-h-screen items-center justify-center px-6 py-12">
        <section class="w-full max-w-md">
            <div class="border-b border-[#dbe3e5] pb-7">
                <p class="text-xs font-bold uppercase tracking-[.2em] text-[#e76f51]">Account recovery</p>
                <h1 class="mt-3 font-serif text-4xl leading-tight tracking-tight">Enter your verification code</h1>
                <p class="mt-4 text-sm leading-6 text-[#607985]">We sent a six-digit code to your email. Enter it below to continue securely.</p>
            </div>
            ...
            <form action="{{ route('password.code.verify') }}" method="POST" class="mt-8 grid gap-5">
                @csrf
                <label class="grid gap-2 text-sm font-semibold" for="email">Email address
                ...
                <label class="grid gap-2 text-sm font-semibold" for="code">Six-digit code
                ...
                <p class="text-xs leading-5 text-[#607985]">The code expires in 10 minutes. Never share it with anyone.</p>
                <button type="submit" data-loading-submit class="flex items-center justify-center gap-2 rounded-xl bg-[#e76f51] px-5 py-3.5 text-sm font-bold text-white shadow-sm transition hover:bg-[#d95f42]"><span data-submit-label>Verify code</span><span data-submit-spinner class="hidden"></span></button>
            </form>

            <a href="{{ route('password.request', ['email' => $email]) }}" class="mt-6 flex items-center justify-center gap-2 text-sm font-semibold text-[#607985] transition hover:text-[#173042]">Request a new code</a>
        </section>
    </main>
</x-auth-layout>
